...
if [[ $? != 0 ]]; then
  echo "cannot find cgexec cmd, please install cg-tools using 'sudo yum install -y libcgroup-tools' first"
  exit 1
fi

set -e
cg_name="bookkeeper"
if [ ! -d "/sys/fs/cgroup/cpu/$cg_name" ]; then
  echo "cannot find controller 'cpu' in group $cg_name"
  exit 1
fi
if [ ! -d "/sys/fs/cgroup/memory/$cg_name" ]; then
  echo "cannot find controller 'memory' in group $cg_name"
  exit 1
fi
