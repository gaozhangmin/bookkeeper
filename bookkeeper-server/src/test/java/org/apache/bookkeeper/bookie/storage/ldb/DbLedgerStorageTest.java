...
public class DbLedgerStorageTest {
    ...
    public void testMultiLedgerDirectoryCheckpoint() throws Exception {
        ...
        LogMark firstLogMark = readLogMark(firstDirMark, false);
        assertEquals(1, firstLogMark.getLogFileId());
        assertEquals(2, firstLogMark.getLogFileOffset());
        LogMark secondLogMark = readLogMark(secondDirMark, true);
        ...
        firstLogMark = readLogMark(firstDirMark, false);
        secondLogMark = readLogMark(secondDirMark, false);
        assertEquals(1, firstLogMark.getLogFileId());
        assertEquals(2, firstLogMark.getLogFileOffset());
        assertEquals(4, secondLogMark.getLogFileId());
        assertEquals(5, secondLogMark.getLogFileOffset());

        // flush in the dbLedgerStorage level, the first and second directory's logMark should be (4, 5);
        bookie.getLedgerStorage().flush();
        firstLogMark = readLogMark(firstDirMark, false);
        secondLogMark = readLogMark(secondDirMark, false);
        assertEquals(4, firstLogMark.getLogFileId());
        assertEquals(5, firstLogMark.getLogFileOffset());
        assertEquals(4, secondLogMark.getLogFileId());
        assertEquals(5, secondLogMark.getLogFileOffset());

        // trigger checkpoint with no ledgerDirsManager set.
    ...
    private LogMark readLogMark(File file, boolean shouldFail) {
        ...
        try {
            ...
            log.error("Failed to read logMark ", e);
            if (!shouldFail) {
                fail();
            }
        }
