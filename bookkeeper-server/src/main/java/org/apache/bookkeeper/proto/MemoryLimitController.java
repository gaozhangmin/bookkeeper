    ...
    private void registerGauges(String gaugeKey, StatsLogger statsLogger) {
        statsLogger.registerGauge(gaugeKey, new Gauge<Number>() {
            ...
            public Number getSample() {
                return getBytesInProgress();
            }
        });

        // Register a usage-ratio gauge (0.0 ~ 1.0+). Only meaningful when limit > 0.
        statsLogger.registerGauge(gaugeKey + BookKeeperServerStats.MEMORY_USAGE_RATIO_SUFFIX, new Gauge<Number>() {
            @Override
            public Number getDefaultValue() {
                return 0;
            }
            ...
            @Override
            public Number getSample() {
                long maxLimit = getMaxBytesLimit();
                if (maxLimit <= 0) {
                    return 0;
                }
                return (double) getBytesInProgress() / maxLimit;
            }
        });
    }
